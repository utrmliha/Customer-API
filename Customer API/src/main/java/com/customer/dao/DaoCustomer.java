	...
	public ApiError Create(CustomerPojo customerPojo) {
		...
		if(!ExistByCpf(customer.getCpf())) {
			...
			handle.createUpdate("INSERT INTO customer "
						+ "(id, uuid, name, email, birthDate, cpf, gender, createdAt, updateAt) VALUES "
						+ "(DEFAULT, NULL, ?, ?, ?, ?, ?, ?, ?)")
				...
				.bind(5, customer.getCreatedAt())
				.bind(6, customer.getUpdateAt())
				.execute();
			
			int idCustomerCriado = handle.createQuery("SELECT id FROM customer WHERE cpf = \"" + customer.getCpf()+"\"").mapTo(int.class).one();
